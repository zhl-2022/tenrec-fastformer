...
class TenrecDataLoader:
    ...
    def _get_cache_path(self) -> Path:
        """生成全量数据缓存文件路径（不包含 max_users）。"""
        # 缓存 key 只基于场景，不含 max_users
        cache_key = f"{self.scenario}_full_v{self.CACHE_VERSION}"
        return self.cache_dir / f"{cache_key}.pkl"
    
    def _get_split_cache_path(self, seed: int) -> Path:
        """生成数据划分缓存文件路径。"""
        cache_key = f"{self.scenario}_split_seed{seed}_v{self.CACHE_VERSION}"
        return self.cache_dir / f"{cache_key}.pkl"
    ...
    def _read_csv_fast(self, csv_path: Path) -> pd.DataFrame:
        ...
        两者都比 csv.DictReader 快 30-100 倍。
        
        Args:
            csv_path: CSV 文件路径
            
        Returns:
            pandas DataFrame（Polars 路径也会转为 pandas）
        """
        ...
        # 填充 NaN 为 0，转换列类型
        # 注意：pd.read_csv 可能将含 NaN 的 int 列推断为 float，需要显式转回 int
    ...
    def load(self) -> "TenrecDataLoader":
        ...
        # 从 CSV 加载（使用 pandas/polars 高速解析）
        logger.info(f"加载 Tenrec 数据: {csv_path}")
        start_time = time.time()
        
        # ===== 高速 CSV 解析 =====
        df = self._read_csv_fast(csv_path)
