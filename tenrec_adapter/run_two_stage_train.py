...
class TenrecDataset(Dataset):
    ...
    def _getitem_fast(self, idx):
        ...
        # 7. Side Information (Video Category, Gender, Age)
        # Video Category: 与 candidate_item_ids 对应
        # 注意: candidate_item_ids 是 hash 后的，我们需要原始 item_id 来查属性
        # 但这里 candidate_item_ids 已经是 hash 值了，无法反推 item_id
        # 必须在 hash 前获取属性

        # 重新梳理流程：
        # item_id -> video_category (原始 ID) -> hash
        # user_id -> gender/age (原始 ID) -> hash

        # 获取原始 item IDs (all_cand 已经被 permutation 打乱了吗？还没有)
        # all_cand: [pos_item, neg_item_1, neg_item_2, ...]

        # 获取对应的 video_category
        # store.video_categories 是全量交互的属性 (N)，但我们需要的是 item 维度的属性
        # store 中存储的是交互时刻的属性，或者我们需要一个 item_id -> category 的映射
        # 由于 store.video_categories 是对齐 interactions 的，我们可以直接取 pos item 的 category
        # 但负样本的 category 呢？Store 里没有负样本的交互记录
        # 因此我们需要 adapter 及其 data_loader 提供 item_id -> category 的全局映射

        # 暂时方案：只使用 user side info (gender, age)，因为它是 user 维度的，可以直接取
        # item side info (category) 需要在 adapter 中构建映射表，目前暂不支持

        user_gender = int(store.genders[store_idx]) if len(store.genders) > 0 else 0
        user_age = int(store.ages[store_idx]) if len(store.ages) > 0 else 0

        # Hash 编码 (假设 gender/age 也是 categorical feature)
        # 这里简单处理：直接作为 raw feature 返回，模型内部 embedding
        # 或者复用 hash 机制： _generate_multi_hash(gender, 1, hs)
        ...
        return {
            ...
            "user_age": torch.tensor(user_age, dtype=torch.long),
        }
